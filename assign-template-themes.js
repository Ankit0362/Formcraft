...
async function main() {
  ...
  const res = await client.query('SELECT id, industry FROM templates ORDER BY industry, created_at');
  ...
  for (const row of res.rows) {
    ...
    // Cycle 1-10 for this industry
    const num = ((industryCounters[prefix] - 1) % 10) + 1;
